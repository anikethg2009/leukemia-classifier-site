...
/* Figures that sit on paper. Violet is a leukemic cell the model flagged,
   rose a leukemic cell it missed, steel a normal cell it flagged in error. */
...
export function mountCellField () {
  ...
  function draw () {
    ...
    for (let i = 0; i < shown; i++) {
      ...
      ctx.beginPath();
      if (d.g === 'fp') {
        /* a normal cell flagged in error: a steel ring */
        ...
        ctx.fillStyle = d.g === 'tp' ? C.violet : d.g === 'fn' ? C.rose : C.faint;
        ctx.fill();
      }
    }
    ctx.globalAlpha = 1;
...
export function mountRoc (THRESHOLD) {
  ...
  for (const t of ticks) {
    ...
    const lx = svg('text', { x: px(t), y: py(0) + 17, fill: CD.soft, 'font-size': 10,
      'text-anchor': 'middle', 'font-family': '"IBM Plex Mono", ui-monospace, monospace' });
    lx.textContent = t.toFixed(1); s.appendChild(lx);

    const ly = svg('text', { x: px(0) - 8, y: py(t) + 3.5, fill: CD.soft, 'font-size': 10,
      'text-anchor': 'end', 'font-family': '"IBM Plex Mono", ui-monospace, monospace' });
  ...
  /* the curve passes under these two labels, so each carries a halo in the
     band colour rather than being moved off the point it describes */
  const halo = { stroke: CD.panel, 'stroke-width': 4, 'paint-order': 'stroke', 'stroke-linejoin': 'round' };
  const opLabel = svg('text', Object.assign({ x: px(ox) + 11, y: py(oy) + 15, fill: CD.mark, 'font-size': 10,
    'font-family': '"IBM Plex Mono", ui-monospace, monospace', 'font-weight': 600 }, halo));
  opLabel.textContent = 'threshold ' + THRESHOLD.toFixed(3);
  s.appendChild(opLabel);
  const opSub = svg('text', Object.assign({ x: px(ox) + 11, y: py(oy) + 28, fill: CD.mark, 'font-size': 9.5,
    'font-family': '"IBM Plex Mono", ui-monospace, monospace' }, halo));
  ...
  const aucLabel = svg('text', { x: px(0.98), y: py(0.06), fill: CD.ink, 'font-size': 11,
    'text-anchor': 'end', 'font-family': '"IBM Plex Mono", ui-monospace, monospace', 'font-weight': 600 });
...
export function mountSplit () {
  ...
  const text = (x, y, str, o = {}) => {
    const t = svg('text', Object.assign({
      x, y, 'font-size': 10.5, fill: C.soft, 'font-family': '"IBM Plex Mono", ui-monospace, monospace'
    }, o));
    t.textContent = str;
    s.appendChild(t);
  };
